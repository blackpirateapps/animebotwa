...
{buttonId: `${prefix}mods`, buttonText: {displayText: 'Owner'}, type: 1}
]
let buttonMessaged = {
...
const inf=` ════ ❃•*Sam Bot 3.0* •❃ ════ 
\`\`\`A FULL FLEDGED WHATSAPP BOT WITH COOL FEATURES\`\`\`
 ═══ ❃• *INFORMATION*•❃ ═══ 
\`\`\`A simple and easy-to-use WhatsApp bot project based on Multi-Device Baileys and written in JavaScript\`\`\`
❁ ══════ ❃• *NOTE* •❃ ══════ ❁
\`\`\`Dont call or spam the bot and before adding make sure you have atleast members in group\`\`\`
❁═════ ❃• *owner* •❃ ═════ 
*_LINK:- wa.me/14318136969_*
❁ ═══ ❃•✍🏻 *ADD BOT IN GROUP* ✍🏻•❃ ═══ ❁
\`\`\`Visit here: sambot.blackpirate.com if you have any feature feel free to contact owner by typing =owner or =mods\`\`\` 
`
let buttonMessagei = {
        image: { url: "https://telegra.ph/file/37289892b8032f795dd0d.png" },
        caption: inf,
 ...
 *Username: Sam Kira Kun*
 *Contact: https://wa.me/14318136969*
━━━━°•°: *Sam Bot 3.0* :°•°━━━━`
const mbut = [
{buttonId: `${prefix}creator`, buttonText: {displayText: 'Creator'}, type: 1},
{buttonId: `${prefix}help`, buttonText: {displayText: 'Help'}, type: 1},
{buttonId: `${prefix}mods`, buttonText: {displayText: 'Sam Bot'}, type: 1}
]
let buttonMessagem = {
        image: { url: "https://telegra.ph/file/37289892b8032f795dd0d.png" },
        caption: mod,
...
try {
  ...
  pfp ='https://telegra.ph/file/37289892b8032f795dd0d.png'
// 
}
...
` 
const buttonsd = [
{buttonId: `${prefix}rank`, buttonText: {displayText: 'Rank'}, type: 1},
{buttonId: `${prefix}help`, buttonText: {displayText: 'Help'}, type: 1}
]
let buttonMessage = {
        image: { url: pfp },
...
arus.sendMessage(m.chat,{sticker:{url: webpWithMetadata}},{quoted:m})
  ...
  case 'toimage': case 'toimg': {
                ...
                exec(`ffmpeg -i ${media} ${ran}`, (err) => {
                    ...
                    arus.sendMessage(m.chat, { image: buffer,caption:'Sam Bot' }, { quoted: m })
                    fs.unlinkSync(ran)
                })
            }    
break
case 'tourl': {
 ...
 case 'togif': {
        ...
        let { webp2mp4File } = require('./lib/uploader')
                ...
                await arus.sendMessage(m.chat, { video: { url: webpToMp4.result, caption: 'Sam Bot' }, gifPlayback: true }, { quoted: m })
                await fs.unlinkSync(media)
            }
            break
...
break
  ...
  case 'group': {
             ...
             let buttons = [
                    ...
                    await arus.sendButtonText(m.chat, buttons, `*Group Open/Close*`, 'Sam Bot', m)
 ...
 case 'waifu':
      ...
      headerType: 4
            ...
            await arus.sendMessage(m.chat,buttonsMessage, { quoted:m }).catch(err => {
                ...
                break  
				case 'waifu18':
                waifudi = await axios.get('https://api.waifu.pics/nsfw/waifu')
              
                var wbutssa = [
        {buttonId: `${prefix}waifu18`, buttonText: {displayText: `NEXT`}, type: 1},
    
        ]
      let buttonsssMessage = {
      ...
      headerType: 4
            ...
            await arus.sendMessage(m.chat,buttonsssMessage, { quoted:m }).catch(err => {
                    return('error..')
                })
                break
				case 'neko18':
                waifudi = await axios.get('https://api.waifu.pics/nsfw/neko')
              
                var wbutssn = [
        {buttonId: `${prefix}neko18`, buttonText: {displayText: `NEXT`}, type: 1},
    
        ]
      let buttonssnMessage = {
       image: await getBuffer(waifudi.data.url),
      ...
      headerType: 4
            ...
            await arus.sendMessage(m.chat,buttonssnMessage, { quoted:m }).catch(err => {
                    return('error..')
                })
                break
				
				case 'succubus':
                waifudi = await axios.get('https://akaneko-api.herokuapp.com/api/succubus')
              
                var wbutssd = [
        {buttonId: `${prefix}succubus`, buttonText: {displayText: `NEXT`}, type: 1},
    
        ]
      let buttonssdMessage = {
       image: await getBuffer(waifudi.data.url),
       caption:  `*Here is your succubus*`,
      footer: 'Sam Bot',
      buttons: wbutssd,
      headerType: 4
      }
            await arus.sendMessage(m.chat,buttonssdMessage, { quoted:m }).catch(err => {
                    return('error..')
                })
                break
				
				case 'yuri':
                waifudi = await axios.get('https://akaneko-api.herokuapp.com/api/yuri')
              
                var wbutssy = [
        {buttonId: `${prefix}yuri`, buttonText: {displayText: `NEXT`}, type: 1},
    
        ]
      let buttonssyMessage = {
       image: await getBuffer(waifudi.data.url),
       caption:  `*Here is your yuri*`,
      footer: 'Sam Bot',
      buttons: wbutssy,
      headerType: 4
      }
            await arus.sendMessage(m.chat,buttonssyMessage, { quoted:m }).catch(err => {
                    return('error..')
                })
                break
				
				case 'school':
                waifudi = await axios.get('https://akaneko-api.herokuapp.com/api/school')
              
                var wbutssl = [
        {buttonId: `${prefix}school`, buttonText: {displayText: `NEXT`}, type: 1},
    
        ]
      let buttonsslMessage = {
       image: await getBuffer(waifudi.data.url),
       caption:  `*Here is your sax*`,
      footer: 'Sam Bot',
      buttons: wbutssl,
      headerType: 4
      }
            await arus.sendMessage(m.chat,buttonsslMessage, { quoted:m }).catch(err => {
                    return('error..')
                })
                break
				
				case 'masturbation':
                waifudi = await axios.get('https://akaneko-api.herokuapp.com/api/masturbation')
              
                var wbutssm = [
        {buttonId: `${prefix}masturbation`, buttonText: {displayText: `NEXT`}, type: 1},
    
        ]
      let buttonssmMessage = {
       image: await getBuffer(waifudi.data.url),
       caption:  `*Here is your sax*`,
      footer: 'Sam Bot',
      buttons: wbutssm,
      headerType: 4
            ...
            await arus.sendMessage(m.chat,buttonssmMessage, { quoted:m }).catch(err => {
                    return('error..')
                })
                break
				
				case 'blowjob':
                waifudi = await axios.get('https://akaneko-api.herokuapp.com/api/blowjob')
	...
	case 'dankmemes':
      ...
      let buttonssmeMessage = {
       image: await getBuffer(meme.data.image),
       caption:  `meme`,
      footer: 'Sam Bot',
      buttons: wbutssme,
      headerType: 4
      }
            await arus.sendMessage(m.chat,buttonssmeMessage, { quoted:m }).catch(err => {
                    return('error..')
                })
                break
				
				case 'gonewild':
                let gonewild = await axios.get('https://nekobot.xyz/api/image?type=gonewild')
              
                var wbutssg = [
        {buttonId: `${prefix}gonewild`, buttonText: {displayText: `NEXT`}, type: 1},
    
        ]
      let buttonssgMessage = {
       image: await getBuffer(gonewild.data.message),
       caption:  `*Here u go*`,
      footer: 'Sam Bot',
      buttons: wbutssg,
      headerType: 4
      }
            await arus.sendMessage(m.chat,buttonssgMessage, { quoted:m }).catch(err => {
                    return('error..')
                })
                break
				
				case 'ass':
                let ass = await axios.get('https://nekobot.xyz/api/image?type=ass')
              
                var wbutsas = [
        {buttonId: `${prefix}ass`, buttonText: {displayText: `NEXT`}, type: 1},
    
        ]
      let buttonsasMessage = {
       image: await getBuffer(ass.data.message),
       caption:  `*Here u go*`,
      footer: 'Sam Bot',
      buttons: wbutsas,
      headerType: 4
      }
            await arus.sendMessage(m.chat,buttonsasMessage, { quoted:m }).catch(err => {
                    return('error..')
                })
                break
				
				case 'pussy':
                let pussy = await axios.get('https://nekobot.xyz/api/image?type=pussy')
              
                var wbutspu = [
        {buttonId: `${prefix}pussy`, buttonText: {displayText: `NEXT`}, type: 1},
    
        ]
      let buttonspuMessage = {
       image: await getBuffer(pussy.data.message),
       caption:  `*Here u go*`,
      footer: 'Sam Bot',
      buttons: wbutspu,
      headerType: 4
      }
            await arus.sendMessage(m.chat,buttonspuMessage, { quoted:m }).catch(err => {
                    return('error..')
                })
                break
  ...
  case 'bcgc': case 'bcgroup': {
                ...
                for (let i of anu) {
                    await sleep(1500)
                      ...
                      let txt = `「 Sam Bot BROADCAST 」\n\n${text}\n\nRegards~${pushname}`
const bcbut=[{buttonId:`${prefix}info`,buttonText:{displayText:"Info"},type:1},
{buttonId:`${prefix}creator`,buttonText:{displayText:"Creator"},type:1},
{buttonId:`${prefix}mods`,buttonText:{displayText:"Sam Bot"},type:1}]
